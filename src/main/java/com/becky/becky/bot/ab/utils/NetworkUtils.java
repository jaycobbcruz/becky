...
public class NetworkUtils {
	...
	public static String responseContent(String url) throws Exception {
		...
		if (url.startsWith("https")) {
			client = getUnsecureHttpClient(new URL(url));
		}
...
// in milliseconds which is the timeout for waiting for data.
	...
	public static String maiServiceUrl(String host, String botid, String index, String input) throws UnsupportedEncodingException {
		return ApplicationProperties.getValue("sraix.url") + "/search?index=" + index + "&query=" + URLEncoder.encode(input, "UTF-8");
	}

	public static HttpClient getUnsecureHttpClient(final URL url) {
		try {
			//trustEveryone(url);
	...
	public static void trustEveryone(final URL url) throws Exception {

		HttpsURLConnection.setDefaultHostnameVerifier((hostname, session) -> hostname.equals(url != null ? url.getHost() : null));
