...
import com.becky.becky.bot.ab.utils.IOUtils;
...
public class Bot {
	 ...
	 */
	public int addCategoriesFromAIML() {
	...
	public int addCategoriesFromAIMLIF() {
		...
		try {
			...
			if (folder.exists()) {
				File[] listOfFiles = IOUtils.listFiles(folder);
				if (MagicBooleans.trace_mode) System.out.println("Loading AIMLIF files from " + aimlif_path);
				for (File listOfFile : listOfFiles) {
					if (listOfFile.isFile()) {
						file = listOfFile.getName();
